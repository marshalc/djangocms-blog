...
class MediaTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.youtube = add_plugin(
            self.post.media, 'YouTubePlugin', language='en',
            url='https://www.youtube.com/watch?v=szbGc7ymFhQ'
        )
        self.vimeo = add_plugin(
            self.post.media, 'VimeoPlugin', language='en',
            url='https://vimeo.com/12915013'
        )
        self.media_text = add_plugin(
            self.post.media, 'TextPlugin', language='en', body='random text'
        )
        self.general_text = add_plugin(
            self.post.content, 'TextPlugin', language='en', body='body text'
        )
